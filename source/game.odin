  ...
  for !rl.WindowShouldClose() {
    ...
    switch game_mode {
      case .LEVEL_EDITOR: {
        ...
        move_up, move_left, move_right, move_down, raise, lower, grow, shrink, reset, save, load, rotate: bool

        if rl.IsKeyPressed(.UP)    { move_up = true }
        if rl.IsKeyPressed(.LEFT)  { move_left = true }
        if rl.IsKeyPressed(.RIGHT) { move_right = true }
        if rl.IsKeyPressed(.DOWN)  { move_down = true }
        if rl.IsKeyPressed(.SPACE) {
          if rl.IsKeyDown(.LEFT_SHIFT) { shrink = true }
          else                         { grow = true }
        }
        if rl.IsKeyPressed(.ENTER) {
          if rl.IsKeyDown(.LEFT_SHIFT) { lower = true }
          else                         { raise = true }
        }
        if rl.IsKeyPressed(.R) { reset = true }
        if rl.IsKeyPressed(.S) && rl.IsKeyDown(.LEFT_CONTROL) { save = true }
        if rl.IsKeyDown(.L) { rotate = true }
        ...
        if ImGui.Begin("Level editor") {
          ...
          ImGui.Dummy({ 30, -1 })
          ImGui.SameLine()
          if ImGui.Button(" up ") { move_up = true }
          if ImGui.Button("left") { move_left = true }
          ImGui.SameLine(80)
          if ImGui.Button("right") { move_right = true }
          ImGui.SameLine(200)
          if ImGui.Button("grow") { grow = true }
          ImGui.SameLine()
          if ImGui.Button("shrink") { shrink = true }
          ImGui.Dummy({ 30, -1 }); ImGui.SameLine()
          if ImGui.Button("down") { move_down = true }

          ImGui.SeparatorText("Height")
          if ImGui.Button("raise") { raise = true }
          ImGui.SameLine()
          if ImGui.Button("lower") { lower = true }
          ImGui.SeparatorText("Level")

          if ImGui.Button("clear") { reset = true }
          ImGui.SameLine()
          if ImGui.Button("save") { save = true }
          ImGui.SameLine()
          if ImGui.Button("load") { load = true }
...

        {
          ...
          t := rl.Vector2{ f32(level_data.size.x)/2, f32(level_data.size.y)/2 }
          ...
          for _, tile in level_data.tiles {
            ...
            size := rl.Vector3{ 1, f32(tile.height), 1 }
          ...
          for pos in level_positions_in_area(level_data, marker_position, marker_size) {
            ...
            position := rl.Vector3{ f32(pos.x), height, f32(pos.y) }
            size     := rl.Vector3{ 1, 0.1, 1 }
...
level_read_from_disk :: proc(level_data: ^Level_Data, file_name: string) -> bool {
  full_path := strings.join({ get_assets_path(), file_name }, "/")
...
draw_cube_texture :: proc(texture: rl.Texture2D, position: rl.Vector3, size: rl.Vector3, color: rl.Color) {
  x := position.x
  y := position.y
  z := position.z
